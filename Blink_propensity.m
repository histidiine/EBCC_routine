function [All_rep_duration, All_start, All_peak, nb_CR, all_CR] = Blink_propensity(data_matrix,log_param, thresh_choice, duration_choice,noise_rep,save_rep,fig_rep,pause_rep,CRwin_start)
...
for step = 1:size(log_param,2) % loop for step (conditioning, extinction)
    ...
    for block = 1:log_param(4,step)/10 % nb of blocks will be 8 for cond and 2 for ext (nb of trial /10)
        ...
        for trial = 1:size(data_matrix,2) % each trial
            ...
            winstart=400;
            limStart = 370*log_param(2,step) /duration_ms;% 370 ms
            limEnd = 400*log_param(2,step) /duration_ms;
            
            % ------------ DETECTOR ------------
            % Building detector:
            %             CRwin_start = 600;
            start_window = 1;
            end_window = round(winstart*log_param(2,step)/duration_ms);
            cr_timewindow=yupper(start_window:end_window,:); % 0 à 400 ms
...
%             end
            ...
            if fig_rep==1
                ...
                plot(time_axis,curr_data,'k','LineWidth',3); % EMG PLOT
                hold on;
                pause;
